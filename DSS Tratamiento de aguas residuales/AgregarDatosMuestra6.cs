...
{
    public partial class AgregarDatosMuestra6 : Form
    {
        public AgregarDatosMuestra6(string ID, string Cliente, string Fecha, string Captador, string Analista, string Observacion)
        {
            InitializeComponent();
            id = ID;
            cliente = Cliente;
        ...
        public AgregarDatosMuestra6(string ID, string Captador, string Analista, string Observacion,
            string temperatura, string ph, string turbiedad, string conductividad,
            string od, string dbo5, string sst, string coli_tot, string coli_fec, string cloro)
        {
            InitializeComponent();
            id = ID;
            captador = Captador;
        ...
        private void InsertarDatosMuestra(string id, string parametro, string resultado)
        {
            EjecutarQuery("INSERT INTO Realizar_estudio (id_muestra,id_punto, id_parametro,resultado) VALUES" +
                "(" + "'" + id + "', 6, " + parametro + ", " + CheckNull(resultado) + ")");
        }
        ...
        private void BtnAgregarDatosM_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                InsertarDatosMuestra(id, "18", textCloro.Text);
        ...
        private void BtnActualizarDatosM_Click(object sender, EventArgs e)
        {
            try
            {
                if (MessageBox.Show("Seguro que desea modificar los datos de la muestra " + id + "?", "Confirmar", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    ActualizarDatosMuestra(id, "18", textCloro.Text);
